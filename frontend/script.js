...
function createObjects() {
    ...
    for(const e of responses) {
        let parking_lot = parking_lots[e.parking_id - 1];
        if (e.busy) {
            if (!parking_lot.scene) {
                loader.load("objects/scene.gltf", gltf => {
                    ...
                    parking_lots[e.parking_id - 1].scene = gltf.scene;
                });
            }
        } else {
            if (parking_lot.scene)
                scene.remove(parking_lot.scene);
                delete parking_lots[e.parking_id - 1].scene;
        }
    }
}
